...
const pastedPath = '/mnt/data/Pasted text(242).txt';
...
function investigationSubtype(label = '') {
  ...
  if (text.includes('grafi') || text.includes('tomografi') || text.includes('ultrason') || text.includes('eko') || text.includes('sintigrafi')) return 'Görüntüleme';
  if (text.includes('biyopsi') || text.includes('histopatoloji') || text.includes('sitoloji') || text.includes('immüno')) return 'Patoloji';
  if (text.includes('kültür') || text.includes('antijen') || text.includes('seroloji')) return 'Mikrobiyoloji';
  return 'Laboratuvar';
}
...
function clueTitle(text = '', index = 0) {
  ...
  if (/(biyopsi|grafi|tomografi|ultrason|sintigrafi|test|kültür|serum|kan|idrar|ekokardiyografi|FAST|BT|ko-oksimetri|laktat|sodyum|kalsiyum|kreatinin|hemoglobin)/i.test(text)) return 'Objektif veri';
  if (/(muayene|duyulur|izlen|palpe|hassas|üfürüm|distandü|nabız|kan basıncı|şişlik|solunum|satürasyon|ateş)/i.test(text)) return 'Muayene bulgusu';
...
function transformCase(source, index) {
  ...
  const id = `v174-new-${String(caseNumber).padStart(3, '0')}-${slugify(source.title)}`;
...
for (const item of allCases) {
  ...
  if (JSON.stringify(item).includes('[object Object]')) problems.push(`${item.id}: [object Object] found`);
}
if (problems.length) {
