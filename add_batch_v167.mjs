...
const pastedPath = '/mnt/data/Pasted text(239).txt';
...
function getBranchId(relatedBranch = '') {
  ...
  if (text.includes('nöroloji') || text.includes('noroloji') || text.includes('acil') || text.includes('toksikoloji') || text.includes('üroloji') || text.includes('ortopedi') || text.includes('psikiyatri') || text.includes('kbb') || text.includes('göz')) return 'minor-rotations';
...
function transformCase(source, index) {
  ...
  return {
    id: `v167-new-${String(41 + index).padStart(3, '0')}-${slugify(source.title)}`,
