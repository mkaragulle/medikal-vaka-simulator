...
const pastedPath = '/mnt/data/Pasted text(237).txt';
...
function getBranchId(relatedBranch = '') {
  ...
  if (text.includes('dahiliye') || text.includes('kardiyoloji') || text.includes('nefroloji') || text.includes('endokrinoloji') || text.includes('hematoloji') || text.includes('romatoloji') || text.includes('gastroenteroloji') || text.includes('enfeksiyon')) return 'internal-medicine';
  if (text.includes('nöroloji') || text.includes('noroloji') || text.includes('acil') || text.includes('ortopedi') || text.includes('üroloji') || text.includes('psikiyatri') || text.includes('kbb') || text.includes('göz')) return 'minor-rotations';
...
function extractSetting(profile = '') {
  ...
  if (text.includes('çocuk nefroloji')) return 'Çocuk nefroloji polikliniği';
...
function parseVitals(physical = '') {
  ...
  const spo2 = text.match(/(?:oksijen satürasyonu|SpO2)\s*(%\s*[0-9]{2,3})/i);
  if (spo2) vitals.SpO2 = spo2[1].replace(/\s+/g, '');
  return vitals;
}
...
function transformCase(source, index) {
  const options = [...source.options].sort((a, b) => String(a.id).localeCompare(String(b.id))).map((o) => o.text);
  const correctOption = source.options.find((o) => o.id === source.correctAnswer)?.text || options[0];
  ...
  return {
    id: `v165-new-${String(21 + index).padStart(3, '0')}-${slugify(source.title)}`,
