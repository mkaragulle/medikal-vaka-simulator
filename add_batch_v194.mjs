...
const pastedPath = '/mnt/data/Pasted text(257).txt';
...
function transformCase(source, index) {
  ...
  const id = `v194-new-${String(caseNumber).padStart(3, '0')}-${slugify(source.title)}`;
...
const newCases = uniquePastedCases.map(transformCase).map((item, idx) => ({ ...item, difficulty: reviewDifficultyForDisplay(item, uniquePastedCases[idx]?.difficulty || item.difficulty) }));
