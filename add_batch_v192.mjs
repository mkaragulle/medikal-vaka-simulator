...
const pastedPath = '/mnt/data/klinik_iq_tus_30_denge_set_31_33_exact_structure(1).txt';
...
function transformCase(source, index) {
  ...
  const id = `v192-new-${String(caseNumber).padStart(3, '0')}-${slugify(source.title)}`;
...
const newCases = uniquePastedCases.map(transformCase);
