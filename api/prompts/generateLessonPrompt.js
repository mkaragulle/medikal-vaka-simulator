...
- Do not start section headings with manual numbering such as 01, 02, Bölüm 1, Section 1. The UI handles structure; headings should contain only the concept name.
...
- clinicalExamRelevance must be one compact, high-value explanatory paragraph. commonConfusions must contain concrete distinctions with one confusion per item; do not turn them into mini lesson sections or duplicate the main prose.
...
- Each section should define the concept, explain how it works, connect it to the broader topic and include why it matters. Put the main explanation in teachingText. Add examAngle/commonTrap only when genuinely specific; otherwise return an empty string. Do not duplicate examAngle/commonTrap sentences inside teachingText. Do not repeat identical sentence openings across sections. Write with a professional teaching tone suitable for a strong medical student. Prefer precise, mechanistic and educational prose over decorative language. Do not return sectionDepthAdequate=true unless at least 80% of sections have detailed teachingText.
