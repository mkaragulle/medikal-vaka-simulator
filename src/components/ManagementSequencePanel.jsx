  ...
  return (
    <article className={`management-step-card advanced-step-card v38-step-card ${inPlan ? 'in-plan' : 'available'} ${stateClass}`.trim()}>
      ...
      <div className="management-step-copy">
        <p><GlossaryText text={step.label} enabled={mode !== 'exam' && !hardMode} revealMode={glossaryRevealMode} maxTerms={5} /></p>
        {submitted ? <small><GlossaryText text={step.rationale} enabled={mode !== 'exam' && !hardMode} revealMode="postAnswer" maxTerms={5} /></small> : null}
      </div>
          ...
          <ol className="management-correct-sequence">
            {requiredSteps.map((step) => (
              <li key={step.id}>
                ...
                <div>
                  <strong><GlossaryText text={step.label} enabled={mode !== 'exam' && !hardMode} revealMode={glossaryRevealMode} maxTerms={5} /></strong>
                  <p><GlossaryText text={step.rationale} enabled={mode !== 'exam' && !hardMode} revealMode="postAnswer" maxTerms={5} /></p>
                </div>
              </li>
            ))}
          </ol>
