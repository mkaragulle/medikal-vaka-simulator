...
const CUSTOM_SCROLLBAR_SELECTOR = [
...
<svg viewBox="0 0 34 34" width="34" height="34" aria-hidden="true" focusable="false" style="display:block;width:34px;height:34px;overflow:visible;background:transparent;border:0;box-shadow:none;filter:none;">
  ...
  <circle data-ki-pointer-dot="true" cx="17" cy="17" r="2.25" fill="rgba(13,148,136,0.98)"></circle>
    ...
    const resolveCustomScrollbarHover = (target) => {
      if (!isElement(target)) return false;
      if (target === lastScrollbarTarget) return cachedOverScrollbarTarget;
      lastScrollbarTarget = target;
      cachedOverScrollbarTarget = Boolean(target.closest(CUSTOM_SCROLLBAR_SELECTOR));
      return cachedOverScrollbarTarget;
    };
    ...
    const move = (event) => {
      ...
      if (nextOverScrollbar !== isOverScrollbar || nextScrollbarDragging !== isScrollbarDragging) {
        ...
        isScrollbarDragging = nextScrollbarDragging;
        setPointerVisual();
      }
