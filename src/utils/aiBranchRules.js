...
export const AI_BRANCH_RULES = [
  rule('pediatrics', ['Çocuk Sağlığı ve Hastalıkları', 'Pediatri', 'Çocuk'], {
    ...
    titles: ['Yüksek ateş ve peteşi', 'Süt çocuğunda hışıltılı solunum', 'Yenidoğanda sarılık örüntüsü', 'İshal sonrası nöbet', 'Kusma ve dehidratasyon', 'Çocukta ense sertliği', 'Döküntü ve ateş', 'Adölesanda kilo kaybı'],
  ...
  rule('obstetrics-gynecology', ['Kadın Hastalıkları ve Doğum', 'Kadın Doğum', 'Jinekoloji', 'Obstetri'], {
    ...
    titles: ['Erken gebelikte ağrı ve kanama', 'Gebelikte hipertansif tablo', 'Pelvik ağrı ve ateş', 'Amenore sonrası kanama', 'Lohusalıkta ateş', 'Adneksiyal ağrı örüntüsü'],
  ...
  rule('internal-medicine', ['İç Hastalıkları', 'Dahiliye'], {
    ...
    examDefaults: ['Genel durum ve volüm bulguları sistemik hastalık açısından değerlendirilir.', 'Kardiyopulmoner ve batın muayenesi ayırıcı tanıya göre yönlendirilir.', 'Klinik bulgular laboratuvar örüntüsüyle birlikte yorumlanır.'],
  ...
  rule('medical-microbiology', ['Tıbbi Mikrobiyoloji', 'Mikrobiyoloji', 'Enfeksiyon'], {
    ...
    presentations: ['Serolojik örüntü yorumu', 'Kültür ve direnç örüntüsü', 'Temas sonrası tarama', 'Menenjit etkeni ayrımı', 'Viral hepatit paneli', 'Fırsatçı enfeksiyon örüntüsü'],
    titles: ['Serolojik örüntü yorumu', 'Kültürde direnç örüntüsü', 'Temas sonrası tarama', 'Menenjit etkeni ayrımı', 'Viral hepatit paneli', 'Fırsatçı enfeksiyon bulgusu'],
  ...
  rule('medical-pharmacology', ['Tıbbi Farmakoloji', 'Farmakoloji'], {
    ...
    presentations: ['İlaç yan etkisi düşündüren tablo', 'Antidot seçimi', 'Reseptör mekanizması', 'İlaç etkileşimi', 'Toksisite örüntüsü'],
    titles: ['İlaç sonrası döküntü', 'Antidot seçimi gerektiren tablo', 'Toksisite örüntüsü', 'Reseptör mekanizması', 'İlaç etkileşimi bulgusu', 'Yan etki düşündüren tablo'],
    riskContexts: ['İlaç veya toksin maruziyeti öyküsü', 'Doz ve zaman ilişkisinin klinik tabloyu belirlemesi'],
    examDefaults: ['Bulgular ilaç maruziyeti, doz ve zaman ilişkisiyle değerlendirilir.', 'Muayene, toksidrom veya yan etki örüntüsüni destekleyecek şekilde yorumlanır.'],
  ...
  rule('medical-biochemistry', ['Tıbbi Biyokimya', 'Biyokimya'], {
    ...
    presentations: ['Metabolik asidoz örüntüsü', 'Enzim eksikliği yorumu', 'Aminoasit yüksekliği', 'Hipoglisemi ve ketoz', 'Üre döngüsü örüntüsü'],
    titles: ['Metabolik asidoz örüntüsü', 'Enzim eksikliği yorumu', 'Aminoasit yüksekliği', 'Hipoglisemi ve ketoz', 'Üre döngüsü bozukluğu', 'Yenidoğan tarama örüntüsü'],
    riskContexts: ['Metabolik veya genetik zeminle ilişkili klinik bağlam', 'Laboratuvar örüntüsünin biyokimyasal yolakla eşleştirilmesi'],
    examDefaults: ['Klinik ipuçları laboratuvar örüntüsü ve metabolik yolakla birlikte yorumlanır.', 'Muayene bulguları enzim eksikliğini doğrudan söylemeden metabolik tabloyu destekler.'],
  ...
  rule('medical-pathology', ['Tıbbi Patoloji', 'Patoloji'], {
    ...
    presentations: ['Histolojik örüntü yorumu', 'İmmünohistokimyasal marker ayrımı', 'Nekroz tipi yorumu', 'Tümör marker örüntüsü'],
    titles: ['Histolojik örüntü yorumu', 'Marker profili ayrımı', 'Nekroz tipi yorumu', 'Biyopsi bulgusu', 'Tümör marker örüntüsü'],
    riskContexts: ['Doku örüntüsü ve marker bilgisinin tanısal bağlamı', 'Patolojik bulgunun klinik karar üzerindeki etkisi'],
    examDefaults: ['Makroskopik ve mikroskopik örüntü birlikte değerlendirilir.', 'İmmünohistokimyasal bulgular ayırıcı tanıda kullanılır.'],
  ...
  rule('physiology', ['Fizyoloji'], {
    ...
    presentations: ['Refleks arkı yorumu', 'Solunum fizyolojisi örüntüsü', 'Böbrek fizyolojisi dengesi', 'Kardiyak debi değişikliği'],
    titles: ['Refleks arkı yorumu', 'Solunum örüntüsü', 'Böbrek dengesi', 'Kardiyak debi değişikliği', 'Asit-baz kompansasyonu'],
  ...
  rule('anatomy', ['Anatomi'], {
    ...
    presentations: ['Sinir lezyonu örüntüsü', 'Damar yaralanması', 'Kas fonksiyon kaybı', 'Dermatom dağılımı'],
    titles: ['Sinir lezyonu örüntüsü', 'Damar yaralanması', 'Dermatom dağılımı', 'Kas fonksiyon kaybı', 'Cerrahi anatomi ipucu'],
  ...
  rule('histology-embryology', ['Histoloji ve Embriyoloji', 'Histoloji', 'Embriyoloji'], {
    ...
    presentations: ['Embriyolojik köken yorumu', 'Doku tipi ayrımı', 'Konjenital anomali örüntüsü', 'Epitel tipi sorusu'],
    titles: ['Embriyolojik köken yorumu', 'Epitel tipi ayrımı', 'Konjenital anomali örüntüsü', 'Doku kesiti bulgusu', 'Germ yaprağı ilişkisi'],
    riskContexts: ['Gelişimsel veya histolojik örüntüsün temel bilim bağlamı'],
  ...
  rule('tus-spot-olgular', ['TUS Spot Olgular', 'TUS Spot', 'Rastgele'], {
    ...
    presentations: ['Kısa klinik olgu', 'Tetkik yorumu', 'İlk yaklaşım kararı', 'Mekanizma bilgisi'],
    titles: ['Kısa klinik olgu', 'Tetkik yorumu', 'İlk yaklaşım kararı', 'Mekanizma bilgisi', 'Ayırıcı tanı ipucu'],
...
export function sanitizeAIQuestionTitle(rawTitle = '', { seed = {}, profile = {}, rule = null, key = '' } = {}) {
  ...
  let title = candidates.length ? pickByHash(candidates, key || seed.seedId || rawTitle) : pickByHash(rule?.titles || ['Kısa klinik olgu'], key || seed.seedId || rawTitle);
  title = title
    .replace(/[.?!;:,]+$/u, '')
    .split(/\s+/)
    .slice(0, 10)
...
export function buildBranchAwareStem(seed = {}, profile = {}, angle = {}, correctText = '') {
  ...
  const maskedTarget = maskAnswerLeak(seed.learningTarget || profile.presentation || 'belirleyici bulgu', correctText);
