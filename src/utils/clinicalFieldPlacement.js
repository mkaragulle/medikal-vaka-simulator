...
export const isInvestigationResult = (text = '') => ['lab', 'imaging'].includes(classifyClinicalDatum(text));
...
function sentenceClean(text = '') {
  ...
  return value ? value.charAt(0).toLocaleUpperCase('tr') + value.slice(1) : '';
}
...
function normalizeEvidenceItem(item = {}, index = 0) {
  ...
  const titleByType = { chiefComplaint: 'Başvuru yakınması', history: 'Öykü', physicalExam: 'Fizik muayene', lab: 'Laboratuvar', imaging: 'Görüntüleme', vital: 'Vital bulgu' };
...
function isAllowedPresentationText(text = '') {
  ...
  if (type === 'physicalExam') return /(ağrı|ateş|baş|güçsüzlük|halsizlik|kusma|ishal|sarılık|idrar|şişlik|nefes|öksürük|kanama|defisit|nörolojik|yürüyememe|ödem|görme|hemiparezi|hassasiyet|uykuya\s+eğilim)/iu.test(text);
  return true;
}
...
function repairPatientIntro(question = {}) {
  ...
  const sourceClues = [...currentClues, ...evidenceClues].map(compactClue).filter(Boolean);
  const fallbackClues = [chiefComplaint, ...(Array.isArray(question.exam) ? question.exam : []), ...(question.investigations || []).map((item) => item.summary || item.label)].map(compactClue).filter(Boolean);
  const clues = dedupeItems([...sourceClues, ...fallbackClues], 5);
  const result = {
    ...intro,
    presentation: sentenceClean(intro.presentation || chiefComplaint),
    ...
    historySummary: sentenceClean(intro.historySummary || question.stem || ''),
    priorityFocus: '',
  };
...
export function repairMisplacedClinicalData(caseItem = {}) {
  ...
  repaired.learningTarget = sentenceClean(repaired.learningTarget || '');
  ...
  repaired.investigations = (Array.isArray(repaired.investigations) ? repaired.investigations : []).map((item, index) => ({
    ...
    rows: Array.isArray(item.rows) ? item.rows.map((row) => Array.isArray(row) ? row.map((cell) => typeof cell === 'string' ? sentenceClean(cell) : cell) : row) : item.rows,
  }));
  ...
  if (repaired.findings) {
    repaired.findings = { ...repaired.findings, history: normalizeArrayItems(repaired.findings.history || []), exam: normalizeArrayItems(repaired.findings.exam || []).filter((item) => !['lab', 'imaging'].includes(classifyClinicalDatum(item))), investigations: repaired.investigations };
  }
  return repaired;
}
...
export function validateClinicalFieldPlacement(caseItem = {}) {
  ...
  [caseItem.chiefComplaint, caseItem.patientIntro?.presentation].forEach((value, index) => {
    ...
    const physicalPresentationAllowed = type === 'physicalExam' && /(ağrı|ateş|baş|güçsüzlük|halsizlik|kusma|ishal|sarılık|idrar|şişlik|nefes|öksürük|kanama|defisit|nörolojik|yürüyememe|ödem|görme|hemiparezi|hassasiyet|uykuya\s+eğilim)/iu.test(raw);
  ...
  (Array.isArray(caseItem.exam) ? caseItem.exam : []).forEach((item, index) => {
    ...
    if (['lab', 'imaging'].includes(type)) errors.push(`exam[${index}] tetkik/görüntüleme verisi içeriyor`);
  });
