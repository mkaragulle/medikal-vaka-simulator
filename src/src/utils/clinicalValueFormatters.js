...
export const normalizeIncompleteLabResult = normalizeLabResultRow;
...
export function validateInvestigationLabCompleteness(investigation = {}) { const normalized = normalizeInvestigationLabResults(investigation), errors = []; (normalized.rows || []).forEach((row, index) => { const validation = validateLabResultCompleteness(row, { label: normalized.label, type: normalized.type }); if (!validation.ok) errors.push(`${normalized.label || 'Tetkik'} satır ${index + 1}: ${validation.errors.join(', ')}`); }); return { ok: errors.length === 0, errors, investigation: normalized }; }
